class AutomatedMetareviewsController < ApplicationController
  ...
  def list
    ...
    avg_existing_metareviews = AutomatedMetareview.find_by_sql(["select avg(relevance) as relevance, avg(coverage) as coverage, avg(content_summative) as summative, 
    ...
    if(!avg_existing_metareviews.nil?)
      ...
      if(avg_existing_metareviews.coverage.nil? or avg_existing_metareviews.coverage < 0)
        avg_existing_metareviews.coverage = 0
      end
    ...
    if(@automated_metareview.coverage.to_f < 0)
      @automated_metareview.coverage = 0
    end
    ...
    current_metareview_data = [@automated_metareview.relevance.to_f, @automated_metareview.coverage.to_f, @automated_metareview.content_summative.to_f , 
    ...
    existing_metareview_data = [avg_existing_metareviews.relevance.to_f, avg_existing_metareviews.coverage.to_f, avg_existing_metareviews.summative.to_f, 
    ...
    names_array = ["Neutral Tone", "Negative Tone", "Positive Tone", "Advisory Content", "Problem Content", "Summative Content", "Coverage", "Relevance"]
    GoogleChart::BarChart.new("500x550", "Your work Vs Average performance on reviews", :horizontal, false) do |bc|
