...
class WordnetBasedSimilarity
  ...
  def compare_strings(reviewVertex, submVertex)
    ...
    for i in (0..stokRev.length-1)
      ...
      if(stokRev[i].nil? or stokSub[i].nil?)
      ...
      #for i in (0..stokSub.length-1)
        ...
        if(check_match(revToken, submToken, revSyn, submSyn, revStem, submStem, reviewState, submState, SYNONYM, ANTONYM))
          next
        end
        #------------------------------------------
        #ANTONYMS
        ...
        if((!revGloss.nil? and !submGloss.nil? and overlap(revGloss, submGloss) > 0) or 
          (!revGloss.nil? and (compare_arrays(revGloss, submTok) or compare_arrays(revGloss, submStem))) or 
          (!submGloss.nil? and (compare_arrays(submGloss, revTok) or compare_arrays(submGloss, revStem))))
        ...
        if((!revExam.nil? and !submExam.nil? and overlap(revExam, submExam) > 0) or 
          (!revExam.nil? and (compare_arrays(revExam, submTok) or compare_arrays(revExam, submStem))) or 
          (!submExam.nil? and (compare_arrays(submExam, revTok) or compare_arrays(submExam, revStem))))
          if(reviewState == submState)
  ...
  if(!lemma.nil? and lemma != "" and !lemma.synsets.nil?)      
    ...
    for g in 0..lemma.synsets.length - 1
      ...
      if(!lemma_synset.gloss.nil?)
        ...
        add_words_from_array(def_arr, extract_definition(lemma_synset.gloss))
      ...
      if(!lemma_synset.gloss.nil?)
        #puts "lemma_synset.gloss.class #{lemma_synset.gloss.class}"
        add_words_from_array(exam_arr, extract_examples(lemma_synset.gloss))
      ...
      if(!lemmaSyns.nil? and lemmaSyns.length != 0)
        ...
        for h in 0..lemmaSyns.length - 1
          ...
          add_words_from_array(syn_arr, lemmaSyns[h].words)
      ...
      if(!lemmaHypers.nil? and lemmaHypers.length != 0)
        ...
        for h in 0..lemmaHypers.length - 1
          ...
          add_words_from_array(hyper_arr, lemmaHypers[h].words)
      ...
      if(!lemmaHypos.nil? and lemmaHypos.length != 0)
        ...
        for h in 0..lemmaHypos.length - 1
          ...
          add_words_from_array(hypo_arr, lemmaHypos[h].words)
      ...
      if(!lemmaAnts.nil? and lemmaAnts.length != 0)
        ...
        for h in 0..lemmaAnts.length - 1
          ...
          add_words_from_array(anto_arr, lemmaAnts[h].words)
        ...
        if(!def2[j].nil?)
          ...
          s1.each do |tok1|
            ...
            s2.each do |tok2|
              ...
              if((tok1.downcase == tok2.downcase or tok1stem.downcase == tok2stem.downcase) and !instance.is_frequent_word(tok1))
