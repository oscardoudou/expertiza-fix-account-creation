#require 'rubygems'
# require 'engtagger'
# require './graphGenerate'
# require './patternIdentify'
# require 'faster_csv'

module TextCollectionNew
  SIM_MATCH = 5
  NumClasses = 1
...
def getReview(flag, textArray)
  ...
  if(flag == 0)
    reviews[0] = Array.new #initializing the array for the *all reviews* in the class
  end
  
  for k in (0..textArray.length-1)
    text = textArray[k][0].to_s
  ...
  if(flag == 0)#training
    ...
    puts("**** Number of review sentences for training:: #{i}")
  else #testing
    numReviews = j
  end
